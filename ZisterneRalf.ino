...
//Emailadresse und PW hinterlegen
EMailSender emailSend("ralfszisterne@gmail.com", "A#dCa3D-;yky");
...
void setup() {
  ...
  ///////////////////////////////////////////Email Nachrichten festlegen
  messageVoll.subject="Zisterne Voll";
  messageVoll.message="Die Zisterne ist ueber 200cm voll";
  messageLeer.subject="Zisterne Leer";
  messageLeer.message="Die Zisterne ist nur noch unter 50cm voll";
  messageAnfrage.subject="Service Anfrage";
  messageAnfrage.message="Der Server ist online und Email funktioniert.";
...
void loop(){
  ...
  if (hour(timeClient.getEpochTime()+j)==2 & k==0){//einmal Täglich abrufen um 12:00Uhr
    ...
    k++;
    if (a[0]<50){//wenn Fuellstand zu niedrig -> Email
      EMailSender::Response resp=emailSend.send(empfaenger, messageLeer);
    }
    if (a[0]>200){//wenn Fuellstand zu hoch -> Email
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if((header.indexOf("GET /7/on") >= 0)&&cont==0) {//Wenn Button aktiv -> Email senden
              cont++; //Das es nicht dauernt abgeht, wird erst durch aktivieren von/5/on wieder auf Null gesetzt
           ...
           for(i=0; i<historie; i++){ //Mach tabelleneinträge für alle einträge (histoire)
            ...
            client.println("</table><a href='/7/on'><img src='https://tse1.mm.bing.net/th?id=OIP._h5ptKPwlk6jR6MdVHmukwHaHa&pid=Api' alt='Email' align='left' style='width:50px;height:50px;'></a><p style='text-align:right'>Die aktuelle Version 1.2 des Quellcodes gibt es <a href='https://github.com/Feeee23/RalfFuelstand/tree/1.2mitEmail' target='_blank' >hier</a></p>");
